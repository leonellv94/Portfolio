...
// -----------navbar--------------------
// btnmenu nav bar already declared - ya declarado
const scrollActive = document.querySelectorAll('.scroll_active')

function scrolls(i) { 
    window.scroll(0, window.pageYOffset + scrollActive[i].getBoundingClientRect().top)
    }


btnmenu.forEach((button, i) => {
    button.addEventListener('click', () => {
        scrolls(i)
    })
})
